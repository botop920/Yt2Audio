...
import { Play, Pause, Mic2, Sparkles, CheckCircle2, ArrowRight, RotateCcw, AlertCircle, Quote, ChevronDown, ChevronUp, Gauge } from 'lucide-react';
...
export const StepVoice: React.FC<StepVoiceProps> = ({ script, onAudioGenerated, onBack }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const base64 = await generateVoiceOver(script, selectedVoice, selectedSpeed);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8">
        ...
        <div className="lg:col-span-12 space-y-6">
            ...
            <div>
                 ...
                 <div className="bg-zinc-900/50 p-1.5 rounded-xl border border-white/5 inline-flex gap-1 w-full md:w-auto overflow-x-auto">
                    {['Slow', 'Normal', 'Fast'].map((speed) => (
                      <button
                        key={speed}
                        onClick={() => setSelectedSpeed(speed)}
                        disabled={isGenerating}
                        className={`flex-1 md:flex-none px-6 py-2.5 rounded-lg text-sm font-medium transition-all duration-300 ${
                          selectedSpeed === speed 
